...
checks() {
  ...
  echo "   the pinned key: both clusters deliver version 1 while the parameter has moved past it (row A)"
...
EOF
  ...
  why=$(cause kp $NS ssm-app-claimant); echo "   second claimant, reason $(esr kp $NS ssm-app-claimant): $why"
  ...
  kp -n external-secrets delete secret aws-credentials >/dev/null; revalidate; T0=$(date +%s)
  ...
  check "the delivered Secret is retained meanwhile" '[ "$(digests kp $NS ssm-app-tls)" = "$(digests kl $NS ssm-app-tls)" ]'
