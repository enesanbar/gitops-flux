...
PY
  chmod 600 "$tmp/tr.json"; $T create -f "$tmp/tr.json" -o jsonpath='authenticated={.status.authenticated} user={.status.user.username} error={.status.error}' 2>&1 | cut -c1-140; echo
}
...
elapsed=0; for wait in 2 15 45 90; do sleep "$wait"; elapsed=$((elapsed+wait)); printf -- '-- +%ss after deletion: TokenReview: ' "$elapsed"; tokenreview; for m in kubernetes-tenant jwt-tenant jwt-tenant-static; do printf '   %-20s %s\n' "$m" "$(login $m)"; done; done
