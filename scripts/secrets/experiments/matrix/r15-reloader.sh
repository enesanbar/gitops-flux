...
echo "$(el) Secret updated -> $(waitfor 120 "klen trellis trellis-secrets TRELLIS_LLM_API_KEY" 48) b64 chars"
echo "$(el) Reloader bumped the api Deployment -> generation $(waitfor 120 "$K -n trellis get deploy trellis-api -o jsonpath='{.metadata.generation}'" $((gen0+1)))"
...
echo "-- rollback the value (this triggers a second restart, the cost of the tool)"; T0=$(date -u +%s); "$V" cli kv rollback -mount=secret-lab -version=1 trellis/llm | grep -E '^version' | sed 's/^/   /'; $K -n trellis rollout status deploy/trellis-api --timeout=240s >/dev/null 2>&1; sleep 60; $K -n trellis rollout status deploy/trellis-api --timeout=240s >/dev/null 2>&1; echo "$(el) after rollback: process llm-key len=$($K -n trellis exec deploy/trellis-api -c api -- sh -c 'echo ${#TRELLIS_LLM_API_KEY}' 2>/dev/null) ready=$(ready)"
