...
echo "$(el) trellis-secrets -> $(waitfor 300 'es trellis trellis-secrets' SecretSyncedError) msg=$(esmsg trellis trellis-secrets)"
...
echo "$(el) trellis-secrets -> $(waitfor 300 'es trellis trellis-secrets' SecretSynced)"; cd "$W" && git status --short scripts/secrets/vault/policies | sed 's/^/git: /'; echo "R2 end=$(now)"
