...
echo "step 3 (dev): commit and push"; git add components/trellis-secrets/external-secrets.yaml && git commit -q -m "feat(trellis): deliver the optional embedding API key from Vault

Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>" && git push 2>&1 | tail -1 | sed 's/^/   /'; echo "   $(el)"
...
echo "ROTATION friction: step A kv put new version; step B wait <= refreshInterval (1m) or force-sync; step C restart (or Reloader)"; T0=$(date -u +%s); openssl rand -base64 33 | tr -d '\n' | "$V" cli kv put -mount=secret-lab trellis/embedding TRELLIS_EMBEDDING_API_KEY=- | grep -E '^version' | sed 's/^/   /'
echo "   Secret updated at $(el): $(waitfor 120 "klen trellis trellis-secrets TRELLIS_EMBEDDING_API_KEY" 60 >/dev/null; klen trellis trellis-secrets TRELLIS_EMBEDDING_API_KEY) b64 chars (natural 1m interval, no force)"
