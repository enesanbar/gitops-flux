...
echo "$(el) vault-0 sealed=$($K -n vault exec vault-0 -c vault -- vault status -format=json 2>/dev/null | jq -r .sealed) store=$(waitfor 200 'store' InvalidProviderConfig)"
...
echo "-- unseal at $(now)"; T0=$(date -u +%s); "$V" unseal | tail -1; echo "$(el) store -> $(waitfor 400 'store' Valid)"; echo "$(el) trellis-secrets -> $(waitfor 400 'es trellis trellis-secrets' SecretSynced)"; echo "R1b end=$(now)"
