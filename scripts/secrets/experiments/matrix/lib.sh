...
# waitfor <max-seconds> <cmd producing a value> <expected>
# Prints "<value> @<elapsed-at-return>". The trailing stamp is the measurement: a bare $(el) on the
# same echo line is expanded BEFORE this function runs (bash expands command substitutions left to
# right), so it reports when the wait STARTED, never when the event happened. Measured 2026-09-22.
waitfor() {
  local max=$1 exp=$3 i v; for i in $(seq 1 $((max/5))); do v=$(eval "$2"); [ "$v" = "$exp" ] && { echo "$v @$(el)"; return 0; }; sleep 5; done; echo "$v(timeout) @$(el)"; return 1; }
