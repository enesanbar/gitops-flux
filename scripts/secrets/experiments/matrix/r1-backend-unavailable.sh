...
# A deleted pod still reports phase Running while terminating, so wait for the REPLACEMENT to seal.
echo "[wait began $(el)] vault-0 replacement sealed: $(waitfor 180 "$K -n vault exec vault-0 -c vault -- vault status -format=json 2>/dev/null | jq -r .sealed" true)"
echo "[wait began $(el)] store -> $(waitfor 400 'store' InvalidProviderConfig)"; echo "[wait began $(el)] trellis-secrets -> $(waitfor 400 'es trellis trellis-secrets' SecretSyncedError) msg=$(esmsg trellis trellis-secrets)"
...
echo "[wait began $(el)] store -> $(waitfor 600 'store' Valid)"; echo "[wait began $(el)] trellis-secrets -> $(waitfor 600 'es trellis trellis-secrets' SecretSynced)"
echo "[wait began $(el)] worker recovered -> $(waitfor 400 "$K -n trellis get deploy trellis-worker -o jsonpath='{.status.readyReplicas}'" 2) ready replicas"
