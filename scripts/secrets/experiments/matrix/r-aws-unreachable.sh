...
restore_coredns() { # idempotent, on the LIVE object: an interrupted run must not leave DNS poisoned
  $K -n kube-system get configmap coredns -o json | python3 -c 'import json,sys,re; cm=json.load(sys.stdin); cm["data"]["Corefile"]=re.sub(r" *hosts \{\n *127\.0\.0\.1 ssm\.[a-z0-9-]+\.amazonaws\.com\n *fallthrough\n *\}\n","",cm["data"]["Corefile"]); [cm["metadata"].pop(k,None) for k in ("resourceVersion","managedFields")]; json.dump(cm,sys.stdout)' | $K replace -f - >/dev/null 2>&1
  $K -n kube-system rollout restart deploy/coredns >/dev/null 2>&1; rm -f /tmp/coredns.json /tmp/coredns.patched.json
}
trap restore_coredns EXIT INT TERM
...
# The first run re-applied a saved copy and FAILED on a resourceVersion conflict (measured
# 2026-09-22), leaving the black hole up for six minutes; restore_coredns edits the live object and
# is also on the EXIT trap, so an interrupt cannot leave cluster DNS poisoned.
restore_coredns; $K -n kube-system rollout status deploy/coredns --timeout=90s >/dev/null
T0=$(date -u +%s); $K -n $NS annotate externalsecret llm-key-only force-sync="$(date +%s%N)" --overwrite >/dev/null; echo "[wait began $(el)] after DNS restored: llm-key-only=$(waitfor 120 "$K -n $NS get externalsecret llm-key-only -o jsonpath='{.status.conditions[0].reason}'" SecretSynced)"
