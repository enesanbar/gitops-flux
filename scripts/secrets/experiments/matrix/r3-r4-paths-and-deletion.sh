...
echo "[wait began $(el)] -> $(waitfor 120 'es trellis trellis-secrets' SecretSyncedError) msg=$(esmsg trellis trellis-secrets)"
...
$K -n trellis patch externalsecret trellis-secrets --type json -p '[{"op":"replace","path":"/spec/data/2/remoteRef/property","value":"TRELLIS_LLM_API_KEY"}]' >/dev/null; sync trellis trellis-secrets; echo "[wait began $(el)] restored -> $(waitfor 120 'es trellis trellis-secrets' SecretSynced)"
...
echo "[wait began $(el)] trellis-secrets -> $(waitfor 120 'es trellis trellis-secrets' SecretSyncedError) msg=$(esmsg trellis trellis-secrets)"; echo "$(el) Retain: LLM key still $(klen trellis trellis-secrets TRELLIS_LLM_API_KEY) b64 chars; ready=$(ready)"
"$V" cli kv undelete -mount=secret-lab -versions=3 trellis/llm | tail -1; sync trellis trellis-secrets; echo "[wait began $(el)] undeleted -> $(waitfor 120 'es trellis trellis-secrets' SecretSynced)"
