...
echo "[wait began $(el)] Secret recreated by ESO -> $(waitfor 120 "klen trellis trellis-secrets TRELLIS_KEK" 60) b64 chars (KEK)"; sleep 3; echo "$(el) api pods during the rollout:"; pods api | sed 's/^/   /'
...
"$V" cli kv rollback -mount=secret-lab -version=1 trellis/llm | grep -E '^version'; sync trellis trellis-secrets; echo "[wait began $(el)] rolled back -> LLM b64len $(waitfor 120 "klen trellis trellis-secrets TRELLIS_LLM_API_KEY" 220)"
