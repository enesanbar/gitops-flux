...
- **An application that changes cluster copies its subtree** (`ssm.sh copy-tree`) and never
  ...
  version *number*, so the copy replays each parameter's history in order, labels included, and the
  pins in the moved manifests name the same bytes; it checks every history before writing anything
  and refuses one that no longer starts at version 1. It carries superseded values too, a leaked one
  included.
...
| `remoteRef.version` on a key that shares a Secret | The guard for the key class when the consumer takes one Secret (§4). | **Measured**: a new version reaches nothing, the other keys keep refreshing (P2, P3); on Parameter Store the same, with the token beside the pinned key following its new version within one 5-second poll (row A), and 0.20.3 delivering the same pinned bytes (`parity/ssm-parity.sh`). Parameter Store keeps a parameter's last 100 versions and does not drop a labelled one: once the labelled version is the oldest of 100, the next write fails with `ParameterMaxVersionLimitExceeded` until the label moves (read in the `PutParameter` reference, not exercised). So label the version a pin names, and take that failure as the prompt to move the pin; `copy-tree` carries labels to the same version numbers. |
