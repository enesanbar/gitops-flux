...
| `components/ssm-app-secrets/` | The application pattern on that store. Every path is `/devops/<cluster>/<namespace>/…` with free nesting below the namespace; an encryption key pinned to one parameter version beside a token that follows the latest; a credential kept as one JSON parameter and read with one `dataFrom.extract`; a certificate read as it is kept in another team's realm (`/dev-generic/` stands in for it here) and rendered as `kubernetes.io/tls`. | Namespace, paths, the key names the consumer expects, the pinned version, and the other team's path for the certificate. |
