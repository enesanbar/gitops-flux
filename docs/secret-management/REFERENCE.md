...
| `components/aws-parameterstore/` | The store of a cluster that is handed one credential: a `ClusterSecretStore` on AWS Parameter Store that reads the delivered Secret (`external-secrets/aws-credentials`, never in this repository) and lists the namespaces that may use it. The list scopes namespaces, not paths (CONVENTIONS.md §2). | The region, the Secret's name, namespace and keys as your platform delivers them, and the namespace list. |
| `components/ssm-app-secrets/` | The application pattern on that store. Every path of its own is `/devops/<cluster>/<namespace>/…` with free nesting below the namespace; an encryption key pinned to one parameter version, sharing a Secret with a token that follows the latest only to show the pin (CONVENTIONS.md §4 gives a key a Secret of its own by default); a credential kept as one JSON parameter and read with one `dataFrom.extract`; a certificate read as it is kept in another team's realm (`/dev-generic/` stands in for it here) and rendered as `kubernetes.io/tls`. | Namespace, paths, the key names the consumer expects, the pinned version, and the other team's path for the certificate. |
